...
export async function GET(request: Request) {
  try {
    ...
    let projetos = await getProjetos(userDept || undefined, papel || undefined);
...
export async function POST(request: Request) {
  try {
    ...
    const novoProjeto = await createProjeto(nome, responsavel || "Não Definido", departamento, dataInicio, dataFim);
    ...
    return NextResponse.json({ error: "Falha ao gravar no banco de dados" }, { status: 500 });
  }
}
